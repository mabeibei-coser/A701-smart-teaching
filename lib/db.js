...
/**
 * 拉取某用户的全部生成记录（按时间倒序），供前端「我的资料」跨浏览器/设备加载历史。
 * 不返回 report_json 里的大图字段由调用方裁剪（图片走 getReportImage 懒加载）。
 */
export function getReportsByUserId(userId) {
  const db = getDb();
  return db
    .prepare(
      `SELECT id, type, topic, case_type, difficulty, report_json,
...
/**
 * 取单条报告的图片（课件=imageDataUrl，互动=补传的 screenshotImage），限定 owner 防越权。
 * 供「我的资料」缩略图按需懒加载，避免列表接口塞入成片 base64。
 */
export function getReportImage(reportId, userId) {
  const db = getDb();
  const row = db
    .prepare("SELECT report_json FROM reports WHERE id = ? AND user_id = ?")
    .get(reportId, userId);
  if (!row) return null;
  try {
    const report = JSON.parse(row.report_json);
    return report.imageDataUrl || report.screenshotImage || null;
  } catch {
    return null;
  }
}
