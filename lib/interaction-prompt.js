...
function getInteractionSystemPrompt(interactionType, difficulty) {
  ...
  // 所有互动类型共用同一份 keyPoints 字段说明。
  // 注意：必须在每个互动类型的 prompt 中显式展开（而非用「同案例分析结构」之类的跨段落引用），
  // 否则讯飞星辰等 LLM 会间歇性忽略该字段，导致前端「暂无核心要点」。
  const keyPointsSchema = `keyPoints字段（必填，三个子字段缺一不可，长度不少于示例）：
{
